...
export class RetailWalletsComponent implements OnInit {
  ...
  getRetailWallets() {
    this.customApi
      .getRetailWalletGetRetailWalletsDetails(this.customerId)
      .subscribe(
        (data) => {
          this.wallets = data;
  ...
  lockWallet(lock: boolean) {
    ...
    dto.lock = lock;
    this.customApi
      .putRetailWalletLockRetailWallets(this.customerId, dto)
      .subscribe((data) => {
        if (data?.data) {
          this.getRetailWallets();
        }
  ...
  public getIcon(assetType: string): string {
    var at: AssetTypes = AssetTypes[assetType];
    var icon = this.assetTypeUtility.icon(at);
    return icon;
  }
}
